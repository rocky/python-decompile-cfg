...
decompile-cfg in parsing the instruction stream. For example
...
Basic Block and Dominator pseudo instructions are inserted into the
instruction stream.  Bit flag values encoded in an operand are
expanding, EXTENDED_ARG value are folded into the following
instruction operand.
...
Type -h for for full help."""
    ...
    try:
        opts, files = getopt.getopt(
            sys.argv[1:], "hVU", ["help", "version", "decompile_cfg"]
        )
