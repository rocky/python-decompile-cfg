...
PYTHON_VERSIONS = frozenset(((3, 10),))
...
def get_scanner(version, is_pypy=False, show_asm=None):
    ...
    if version[:2] in PYTHON_VERSIONS:
        ...
        try:
            ...
            if is_pypy:
                scan = importlib.import_module("decompile_ng.scanners.pypy%s" % v_str)
            else:
                scan = importlib.import_module("decompile_ng.scanners.scanner%s" % v_str)
            if False:
                print(scan)  # Avoid unused scan
        except ImportError:
            if is_pypy:
                exec(
                    "import decompile_ng.scanners.pypy%s as scan" % v_str,
                    locals(),
                    globals(),
                )
            else:
                exec(
                    "import decompile_ng.scanners.scanner%s as scan" % v_str,
                    locals(),
                    globals(),
                )
        if is_pypy:
...
if __name__ == "__main__":
    import inspect, decompile_ng

    co = inspect.currentframe().f_code
    from xdis.version_info import PYTHON_VERSION_TRIPLE

    scanner = get_scanner(decompile_ng.PYTHON_VERSION_TRIPLE, IS_PYPY, True)
