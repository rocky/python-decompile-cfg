...
class Python310Parser(Python310LambdaParser):
    ...
    def p_import310(self, args):
        """
        stmt          ::= import_as310
        import_as310   ::= LOAD_CONST LOAD_CONST importlist310 store POP_TOP

        importlist310  ::= importlist310 ROT_TWO IMPORT_FROM
        importlist310  ::= importlist310 ROT_TWO POP_TOP IMPORT_FROM
        importlist310  ::= importattr310
        importattr310  ::= IMPORT_NAME_ATTR IMPORT_FROM
        ...
        stmt          ::= import_from310
        importlist310  ::= importlist310 alias310
        importlist310  ::= alias310
        alias310       ::= IMPORT_NAME store
        alias310       ::= IMPORT_FROM store
        import_from310 ::= LOAD_CONST LOAD_CONST IMPORT_NAME_ATTR importlist310 POP_TOP
    ...
    def p_310async(self, args):
        """
        stmt     ::= async_for_stmt310
        stmt     ::= async_for_stmt
        ...
        async_for_stmt310   ::= setup_loop expr
                               GET_AITER
                               _come_froms
    ...
    def p_grammar(self, args):
        ...
        testfalsec ::= c_compare_chained1b_false_310

        testtrue   ::= expr_pjit
        testtruec  ::= expr POP_JUMP_IF_FALSE_BACK
        # Do we have to check the c_compare_chained310 ends in a POP_JUMP_IF_FALSE_BACK?
        testtruec  ::= c_compare_chained310_false
        testtruec  ::= c_compare_chained310
        testtruec  ::= c_nand

        testtrue   ::= compare_chained310
        ...
        testfalse  ::= and_not
        testfalse  ::= not_or
        testfalse  ::= compare_chained310_false
        testfalse  ::= or_cond
        testfalse  ::= or_cond1
        ...
        c_except_handler310 ::= COME_FROM_EXCEPT c_except_stmts END_FINALLY
        c_try_except310     ::= SETUP_EXCEPT suite_stmts_opt POP_BLOCK
                               c_except_handler310 come_from_opt
        c_try_except310     ::= SETUP_EXCEPT returns
                               c_except_handler310 come_from_opt
    ...
    def p_3try_except(self, args):
        ...
        except_handler310 ::= COME_FROM_EXCEPT except_stmts END_FINALLY

        stmt             ::= try_except310
        try_except310     ::= SETUP_EXCEPT returns except_handler310
                             opt_come_from_except
        try_except310     ::= SETUP_EXCEPT suite_stmts
        try_except310     ::= SETUP_EXCEPT suite_stmts_opt POP_BLOCK
                             except_handler310 come_from_opt

        # 3.6 omits END_FINALLY sometimes
        except_handler310 ::= COME_FROM_EXCEPT except_stmts
        except_handler310 ::= JUMP_FORWARD COME_FROM_EXCEPT except_stmts
        except_handler   ::= jmp_abs COME_FROM_EXCEPT except_stmts

        stmt             ::= tryfinally310
        tryfinally310     ::= SETUP_FINALLY returns
...
class Python310FullParser(Python310Parser, Python310LambdaParser):
    ...
    def p_310misc(self, args):
        """
        stmt               ::= async_for_stmt310
        stmt               ::= async_forelse_stmt310
        stmt               ::= for310
        stmt               ::= forelsestmt310
        stmt               ::= forelselaststmt310
        stmt               ::= forelselaststmtc310
        stmt               ::= tryfinally310stmt
        stmt               ::= tryfinally310rstmt
        stmt               ::= tryfinally310rstmt2
        stmt               ::= tryfinally310rstmt3
        stmt               ::= tryfinally310rstmt4
        stmt               ::= tryfinally310astmt
        stmt               ::= try_elsestmtl310
        stmt               ::= try_except_ret310
        stmt               ::= try_except_ret310a
        stmt               ::= try_except310
        stmt               ::= try_except310r
        stmt               ::= try_except_as
        stmt               ::= whilestmt310
        stmt               ::= whileTruestmt310
        stmt               ::= call_stmt
        ...
        async_for_stmt310   ::= expr async_for
                               store for_block
                               COME_FROM_FINALLY
                               END_ASYNC_FOR
        ...
        async_forelse_stmt310 ::= expr async_for
                                store for_block
                                COME_FROM_FINALLY
                                END_ASYNC_FOR
                                else_suite
        ...
        for310              ::= expr get_iter store for_block JUMP_BACK _come_froms
        for310              ::= expr get_for_iter store for_block JUMP_BACK _come_froms
        for310              ::= expr get_for_iter store for_block JUMP_BACK _come_froms POP_BLOCK
        for310              ::= expr get_for_iter store for_block _come_froms

        forelsestmt310      ::= expr get_for_iter store for_block POP_BLOCK else_suite
        forelsestmt310      ::= expr get_for_iter store for_block JUMP_BACK _come_froms else_suite

        c_stmt             ::= c_forelsestmt310
        c_stmt             ::= pop_tops return
        c_forelsestmt310    ::= expr get_for_iter store for_block POP_BLOCK else_suitec
        c_forelsestmt310    ::= expr get_for_iter store for_block JUMP_BACK _come_froms else_suitec

        forelselaststmt310  ::= expr get_for_iter store for_block POP_BLOCK else_suitec
        forelselaststmtc310 ::= expr get_for_iter store for_block POP_BLOCK else_suitec

        whilestmt310        ::= _come_froms testexpr c_stmts_opt COME_FROM JUMP_BACK POP_BLOCK
        whilestmt310        ::= _come_froms testexpr c_stmts_opt JUMP_BACK POP_BLOCK
        whilestmt310        ::= _come_froms testexpr c_stmts_opt JUMP_BACK come_froms
        whilestmt310        ::= _come_froms testexpr returns               POP_BLOCK
        whilestmt310        ::= _come_froms testexpr c_stmts     JUMP_BACK _come_froms
        whilestmt310        ::= _come_froms testexpr c_stmts     come_froms
        ...
        whileTruestmt310    ::= _come_froms c_stmts JUMP_BACK _come_froms
        whileTruestmt310    ::= _come_froms c_stmts JUMP_BACK COME_FROM_EXCEPT_CLAUSE
        ...
        try_elsestmtl310    ::= SETUP_FINALLY suite_stmts_opt POP_BLOCK
                               except_handler310 COME_FROM
                               else_suitec opt_come_from_except
        ...
        try_except310       ::= SETUP_FINALLY POP_BLOCK POP_TOP suite_stmts_opt
                               except_handler310a

        # suite_stmts has a return
        try_except310       ::= SETUP_FINALLY POP_BLOCK suite_stmts
                               except_handler310b
        try_except310r      ::= SETUP_FINALLY return_except
                               except_handler310b
        ...
        try_except_ret310   ::= SETUP_FINALLY returns except_ret310a
        try_except_ret310a  ::= SETUP_FINALLY returns except_handler310c
        ...
        except_ret310       ::= SETUP_FINALLY expr ROT_FOUR POP_BLOCK POP_EXCEPT
                               CALL_FINALLY RETURN_VALUE COME_FROM
                               COME_FROM_FINALLY
                               suite_stmts_opt END_FINALLY
        except_ret310a      ::= COME_FROM_FINALLY POP_TOP POP_TOP POP_TOP
                               expr ROT_FOUR
                               POP_EXCEPT RETURN_VALUE END_FINALLY

        except_handler310   ::= jump COME_FROM_FINALLY
                               except_stmts END_FINALLY opt_come_from_except
        except_handler310a  ::= COME_FROM_FINALLY POP_TOP POP_TOP POP_TOP
                               POP_EXCEPT POP_TOP stmts END_FINALLY
        except_handler310b  ::= COME_FROM_FINALLY POP_TOP POP_TOP POP_TOP
                               POP_EXCEPT returns END_FINALLY
        except_handler310c  ::= COME_FROM_FINALLY except_cond1a except_stmts
                               COME_FROM
        except_handler310c  ::= COME_FROM_FINALLY except_cond1a except_stmts
        ...
        tryfinally310rstmt  ::= sf_pb_call_returns
                               cf_cf_finally
                               ss_end_finally
        tryfinally310rstmt  ::= sf_pb_call_returns
        ...
        tryfinally310rstmt2 ::= lc_setup_finally POP_BLOCK call_finally_pt
                               returns
                               cf_cf_finally pop_finally_pt
                               ss_end_finally POP_TOP

        tryfinally310rstmt3 ::= SETUP_FINALLY expr POP_BLOCK CALL_FINALLY RETURN_VALUE
                               COME_FROM COME_FROM_FINALLY
                               ss_end_finally

        tryfinally310rstmt4 ::= lc_setup_finally suite_stmts_opt POP_BLOCK
        ...
        tryfinally310stmt   ::= SETUP_FINALLY suite_stmts_opt POP_BLOCK
                               BEGIN_FINALLY COME_FROM_FINALLY
                               POP_FINALLY suite_stmts_opt END_FINALLY

        tryfinally310astmt  ::= LOAD_CONST SETUP_FINALLY suite_stmts_opt POP_BLOCK
                               BEGIN_FINALLY COME_FROM_FINALLY
...
def info(args):
    ...
    if len(args) > 0:
        arg = args[0][:2]
        if arg == (3, 10):
            from decompile_ng.parser.parse310 import Python310Parser
